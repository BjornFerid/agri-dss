...
def load_rf_model():
    ...
    print("[ML] No RF model — using random predictions")
    return None
...
def run_inference(payload):
    return {
        ...
        "sensor_data": {
            "N":payload.get("N",0),"P":payload.get("P",0),"K":payload.get("K",0),
    ...
    while True:
        ...
        if not active:
            ...
            try:
                ...
                print(f"[SIM] → {decision['crop_recommendation']['crop']}")
            except Exception as e:
                print(f"[SIM] Publish failed: {e}")

def start_background():
    """Called once — safe under gunicorn multi-worker via lock."""
    global _started
    with _start_lock:
        if _started:
            return
...
# ── Start on first request (works with gunicorn) ───────────────────────────
@app.before_request
